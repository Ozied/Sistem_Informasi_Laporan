...
class wordGenerator{
    ...
    public function generate($data = [])
    {
        try{
            ...
            $section->addText("Puji dan syukur kehadirat Allah SWT, berkat rahmat serta karunianya, laporan " . $pelatihan->nama_pelatihan . " di Wilayah Kerja Kantor Kementerian Agama" . $pelatihan->kab_kota . "ini telah dapat disusun.", $fontstyle, $paragraphstyle);
            $section->addText("Laporan pelatihan ini terdiri dari tiga bab. Bab I Pendahuluan, memuat organisasi diklat, nama unit/satuan kerja, nama diklat yang diselenggarakan, dasar hukum, sumber pembiayaan, susunan panitia, dan alamat penyelenggara. Bab II berisi tentang pelaksanaan " . $pelatihan->nama_pelatihan . "yang mencakup tujuan dan sasaran, kurikulum, peserta, widyaiswara/narasumber, evaluasi, penyelenggaraan, keuangan, penjaminan mutu, dan lain-lain. Bab III sebagai penutup terdiri dari Laporan Keuangan / SPJ Akhir Pelatihan ini. Laporan ini juga dilengkapi dengan lampiran-lampiran sebagai bukti fisik.", $fontstyle, $paragraphstyle);
            $section->addText("Diharapkan bahwa dengan selesainya laporan ini, semua kegiatan yang berhubungan dengan " . $pelatihan->nama_pelatihan . "dapat dipertanggungjawabkan.", $fontstyle, $paragraphstyle);
           ...
           //LAPORAN PENYELENGGARAAN//
            ...
            $style = generate_list_style($phpword, 'alphaList');
            ...
            $section->addListItem("d.	Menciptakan kesamaan visi, dinamika pola pikir dan mengembangkan  sinergi dalam melaksanakan tugas pemerintah umum dan pembangunan demi terwujudnya kepemerintahan yang baik dan bersih.", 0, $fontstyle, $style);
            ...
            $section->addText("Adapun sasaran Pelatihan $pelatihan->nama_kegiatan Tahun $pelatihan->tahun ini adalah tersedianya $pelatihan->jumlah_peserta orang $pelatihan->jabatan_peserta yang cakap dan kompeten.", $fontstyle, $paragraphstyle);
